-- MC-00: disambiguate persisted column references in the existing Market Core baseline.
-- Only relation qualification changes; preserve business logic, counters, and legacy IDs.
-- CREATE OR REPLACE retains existing ownership and EXECUTE grants.
...
begin
  ...
  select i.result into prior
  from public.market_core_idempotency as i
  where i.scope = 'cancel' and i.key = p_idempotency_key;
  if prior is not null then
    return prior;
  end if;
...
begin
  ...
  select i.result into prior
  from public.market_core_idempotency as i
  where i.scope = 'settlement_submit' and i.key = p_idempotency_key;
  if prior is not null then
    return prior;
  end if;
