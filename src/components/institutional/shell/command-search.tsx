...
import {
  ...
  instrumentShellBaseFromPathname,
  instrumentShellHref,
...
export function CommandSearch({
  ...
  const basePath = instrumentShellBaseFromPathname(pathname);
  ...
  const destinations = useMemo(() => {
    const extra =
      instrumentMatch && basePath
        ? INSTRUMENT_SHELL_TABS.map((tab) => ({
            key: `tab-${tab}`,
            href: instrumentShellHref(instrumentMatch[1]!, tab, basePath),
            label: tabLabels[tab],
          }))
        : [];
    return [...items.map((item) => ({ key: item.key, href: item.href, label: item.label })), ...extra];
  }, [items, instrumentMatch, basePath, tabLabels]);
  ...
  return (
    ...
    >
      <button
        ...
        className="flex h-10 w-full items-center gap-2 rounded-md border border-border bg-card px-3 text-left text-[13px] text-muted-foreground hover:border-[#B9C3BC] focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-none"
      >
