...
export async function InstrumentOverviewScreen({
  ...
  const tabItems = INSTRUMENT_SHELL_TABS.map((id) => ({
    href: instrumentShellHref(instrument.id, id),
  ...
  const issuedHint =
    model.issuedSupply !== null ? t("instrumentsIssued") : undefined;
    ...
    <div>
      ...
      <EntityHeader
        ...
        badges={
          <>
            {model.protocolInvestment ? (
              <>
                <StatusChip family="lifecycle" code="STRUCTURING" label={t("statusStructuring")} />
                <StatusChip family="maturity" code="CONCEPT" label={t("noOffering")} />
              </>
            ) : (
              <>
                <StatusChip
                ...
                />
                <StatusChip family="maturity" code="DEMO" label={t("demoMarket")} />
              </>
            )}
          </>
        }
        context={
          <>
            {protocol ? (
              <ProtocolBadge assetClass={instrument.assetClass}>
                {protocol.name}
                {" / "}
      ...
      {!model.protocolInvestment ? (
        <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            hint={
              model.coverage
                ? t("coverageOfGross", {
                    ...
                    gross: formatInteger(model.coverage.grossVolumeTonnes, locale),
                  })
                : undefined
            }
          ...
          <MetricCard
            label={t("metricUnit")}
            value={t("unitEquals")}
            hint={model.unitLabel ?? undefined}
            icon={<Scale className="size-4" />}
          />
          <MetricCard
            label={t("metricCurrency")}
            value={model.settlementCurrency ?? t("notAvailable")}
            hint={t("unitOfAccountHint")}
            icon={<Landmark className="size-4" />}
          />
        </div>
      ) : null}

      <ContextTabs ariaLabel={t("localNav")} items={tabItems} />

      {tab === "overview" ? <OverviewGrid model={model} locale={locale} chain={chain} /> : null}
      ...
      {tab === "market" ? <MarketPanel model={model} locale={locale} /> : null}
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-4 xl:grid-cols-12">
        <EntityCard title={t("instrumentSummary")} className="xl:col-span-3">
          <FactList
            items={[
              {
                label: t("assetClass"),
        ...
        <EntityCard title={t("termsGlance")} className="xl:col-span-3">
          {model.protocolInvestment ? (
        ...
        <EntityCard
          ...
          action={
            market ? (
              <Link
                href={model.marketWorkspaceHref}
              ...
              items={[
                {
                  ...
                  value: model.bookAvailable
                    ? model.lastTrade
                      ? formatNumber(model.lastTrade.price, locale, 3)
                      : t("noTrades")
                    : t("bookUnavailable"),
                },
                {
                  label: t("bestBid"),
                  value: model.bestBid
                    ? formatNumber(model.bestBid.price, locale, 3)
                    : t("none"),
                },
                {
                  label: t("bestAsk"),
                  value: model.bestAsk
                    ? formatNumber(model.bestAsk.price, locale, 3)
                    : t("none"),
                },
                {
                ...
                },
                {
                  label: t("openOrders"),
                  value:
                    model.openOrderCount === null
                      ? t("bookUnavailable")
                      : formatInteger(model.openOrderCount, locale),
                },
              ]}
            />
          ) : (
      ...
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">
        ...
        <RecentActivity model={model} locale={locale} />
      </div>
    </div>
  );
}
...
async function OwnershipPanel({
  ...
  return (
    <EntityCard
      ...
      description={t("registeredOwnershipNote")}
