...
export function EconomicBasisPanel({
  ...
  return (
    <EntityCard title={title}>
      <ol
        className={cn(
          "mb-4 flex flex-wrap items-center gap-x-1.5 gap-y-2",
      ...
      >
        {chain.map((step, index) => (
          <li key={`${step}-${index}`} className="flex items-center gap-1.5">
            ...
            <span
              className={cn(
                "rounded-md border border-border bg-[#F7F8F5] px-1.5 py-0.5 font-medium text-foreground",
      ...
      {coverage ? (
        <dl className={cn("grid gap-x-4 gap-y-3", compact ? "grid-cols-2" : "grid-cols-2 sm:grid-cols-4")}>
          <div>
      ...
      <div
        className={cn(
          "mt-4 flex gap-2.5 rounded-md border border-[#0B5D3B]/15 bg-[#EAF4EE] px-3 py-2.5",
          compact && "mt-3 px-2.5 py-2",
        )}
      >
        <Leaf className="mt-0.5 size-4 shrink-0 text-[#0B5D3B]" aria-hidden />
        <p className={cn("leading-relaxed text-[#084A30]", compact ? "text-[11px]" : "text-[12px]")}>
          {callout}
        </p>
      </div>
    </EntityCard>
