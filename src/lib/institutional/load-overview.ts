...
export interface LoadInstrumentOverviewOptions {
...
export async function loadInstrumentOverview(
  instrumentId: string,
  actor: ActorContext,
  options?: LoadInstrumentOverviewOptions,
  ...
  const engine = wheat
    ? options && "engine" in options
      ...
      : await loadEngine()
    : null;
  const snapshot: PlacementSnapshot | null = wheat
    ? options && "placement" in options
      ? (options.placement ?? null)
      : await getPlacementSnapshot()
    : null;
  const issuedSupply =
